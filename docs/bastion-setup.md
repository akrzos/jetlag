...
This page covers the initial setup of the bastion machine for all Jetlag deployments. Complete these steps before following any deployment guide.

1. Select the bastion machine from the allocation. You should run Jetlag on the
bastion machine, to ensure full connectivity and fastest access. In Scale Lab and
Performance Lab environments, the first machine listed in the cloud's
`ocpinventory.json` file is used as the bastion by default. You can find your
allocation's machine list on the lab wiki. For IBMcloud, the bastion is typically
the first server listed by `ibmcloud sl hardware list`.

## Choosing a different bastion machine

By default Jetlag selects machines for the roles bastion, control-plane, and worker in that order from the lab's `ocpinventory.json` file. If you want a different machine as the bastion, download the ocpinventory JSON, reorder the entries so your preferred bastion is first, and set the `ocp_inventory_override` variable to point to your modified file:

```yaml
ocp_inventory_override: http://<http-server>/<inventory-file>.json

# or use a local path on the bastion
ocp_inventory_override: <LOCAL_FILE_PATH>
```

Set this variable in the "Extra vars" section of `ansible/vars/all.yml` before running `create-inventory.yml`.
